...
{
	uint8_t id[OW_ROMCODE_SIZE], sp[DS18X20_SP_SIZE];
	uint8_t i;
	uint16_t meas;
	int16_t decicelsius;
	char s[10];
	...
	for( diff = OW_SEARCH_FIRST; diff != OW_LAST_DEVICE; )
	{
        ...
        DS_PWR_OUT_LOW();
#endif		      
		if(!(OWSearch() && OWVerify())) break;   
#ifdef DS_PWR_PIN
        DS_PWR_OUT_HIGH();
#endif		   
				for ( i=0; i < OW_ROMCODE_SIZE; i++ )
									id[i] = ROM_NO[i];
				if(++nSensors >= MAXSENSORS ) break;
...
uint8_t DS18X20_find_sensor( uint8_t *diff, uint8_t id[] )
{
	...
	do {
		...
		*diff = ow_rom_search( *diff, &id[0] );
	     DOINTERRUPT();
		if (*diff == OW_PRESENCE_ERR || *diff == OW_LAST_DEVICE || *diff == OW_DATA_ERR) {
				ret = DS18X20_ERROR; break;
...
//zh		_delay_ms(DS18X20_COPYSP_DELAY); // wait for 10 ms 
#ifdef DS_PWR_PIN
		send_power_10ms();
#endif
#ifndef	DS_PWR_PIN
