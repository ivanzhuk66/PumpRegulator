...
# 86 "/AtmelAvrToolchain/avr/include/avr/pgmspace.h" 3
...
# 87 "/AtmelAvrToolchain/avr/include/avr/pgmspace.h" 2 3
...
# 89 "/AtmelAvrToolchain/avr/include/avr/pgmspace.h" 2 3
...
int (*gsm_get_byte)(void) = uart_getc;
void (*gsm_send_byte)(unsigned char c) = uart_putc;
...
int gsm_check_new_sms( SMS *sms ){
 ...
 for(i=0; i<200; i++){
  if( gsm_readline(line, sizeof(line)) == 1 ){
   if( strstr(line, "REC UNREAD") != ((void *)0) ){
...
int gsm_read_sms( int index, SMS *sms ){
 ...
 uart_puts_p((__extension__({static const char __c[] __attribute__((__progmem__)) = ("AT+CMGR="); &__c[0];})));
 uart_put_int(index);
 uart_puts_p((__extension__({static const char __c[] __attribute__((__progmem__)) = ("\n"); &__c[0];})));
 DoWatchDog();
 _delay_ms(1000);
 DoWatchDog();
 ...
 for(i=0; i<200; i++){
  if( gsm_readline(line, sizeof(line)) == 1 ){
   if( strstr(line, "+CMGR") != ((void *)0) ){




    if( strstr(line, "REC READ") != ((void *)0) )
     sms->stat = 1;
    else if( strstr(line, "REC UNREAD") != ((void *)0) )
     sms->stat = 0;
    ...
    while( gsm_readline(line, sizeof(line)) == 0 );
...
int gsm_send_sms( char *number, char *text, int try_num ){
 ...
 for( try=0; try<=try_num; try++ ){
  ...
  uart_puts_p((__extension__({static const char __c[] __attribute__((__progmem__)) = ("AT+CMGS=\""); &__c[0];})));
  ...
  for(i=0; i<50; i++){
   ...
   DoWatchDog();
   _delay_ms(100);
   if( gsm_readline(line, sizeof(line)) != 1 ) continue;
   ...
   if( strstr(line, "ERROR") != ((void *)0) ) return 0;

   if( strstr(line, "+CMGS") != ((void *)0) ){
    gsm_read_token(line, token, sizeof(token), 1, ":");
    refnum = atoi(token);
    if( try_num == 0 )
     return refnum;
  ...
  if( refnum <= 0 ) return 0;
   ...
   if( strstr(line, "+CDS: 6") == ((void *)0) ) continue;
...
int gsm_del_sms( int index ){
 ...
 uart_puts_p((__extension__({static const char __c[] __attribute__((__progmem__)) = ("AT+CMGD="); &__c[0];})));
 uart_put_int(index);
 uart_puts_p("\n");
...
int gsm_read_token( char *src, char *dest, char lenght, char nToken, char *delimiter ){
  ...
  if( token != ((void *)0) && strlen(token) < lenght){
    strcpy(dest, token);
    free( temp_p );
    return 0;
  }
